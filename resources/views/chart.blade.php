                                 ...
                                 <select class="form-control price_type" data-id="{{ $key }}" id="price_type_{{ $key }}" name="price_item[]">
                                  @php $t = " " @endphp
                                    ...
                                    <option data-type="{{ $v->type }}" data-amount="{{ $v->amount }}" data-price="{{ $v->price }}" value="{{ $v->price }}"> Rp. {{ number_format($v->price) }} / {{ $v->amount }}
                                       ...
                                       @switch($v->type)
                                       	@case(1)
                                       		 @php $type_time = "Hari" @endphp
                                       		Hari    
                                       	@break
	                                       @case(2)
	                                       	@php $type_time = "Minggu" @endphp
	                                       	Minggu
	                                       @break
	                                       @case(3)
	                                       	@php $type_time = "Bulan" @endphp
	                                       	Bulan
	                                       @break
	                                       	@default
	                                       	Hari
                                       @endswitch
                                    	...
                                    	@if($k == 0)
                                    		@php $t = $v->amount." ".$type_time @endphp
                                    	@endIF
                                    @endForeach
                                 </select>
                                 <input type="hidden" name="time_rent[]" id="time_rent_{{ $key }}" value="{{ $t }}">
                              </div>
                              <div class="col-md-6 mt-20">
                                 <span>Deposit</span>
   ...
   $(function() {
   	...
   	@endIf
   		...
   		$(".price_type").change(function(){
   			let id = $(this).data('id');
   			let variable = $(this).find(":selected");
   			let type = parseInt(variable.data('type'));
   	      let amount = parseInt(variable.data('amount'));
   	      $type_time = 'Hari';
